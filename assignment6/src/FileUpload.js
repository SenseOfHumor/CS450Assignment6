...
class FileUpload extends Component {
  state = { file: null };
  parsedData = null; // to store the csv data to pass to chart
      ...
      reader.onload = (e) => { // Define the onload event handler
        ...
        d3.csv(url).then((data) => { // Use d3.csv to parse the CSV data (asynchronously)
          const formattedData = data.map(d => ({ // Format the data
            Date: d3.timeParse("%m/%d/%y")(d.Date), // parse date as obj
            "GPT-4": d['GPT-4'],
            "Gemini": d['Gemini'],
            "PaLM-2": d['PaLM-2'],
            "Claude": d['Claude'],
          ...
          this.setState({parsedData: formattedData}) // going to the state
  ...
  render = () => (
    <div style={{ backgroundColor: "#f0f0f0", padding: 20 }}>
      ...
      {/* {this.state.parsedData && <Chart data={this.state.parsedData} />} */}
